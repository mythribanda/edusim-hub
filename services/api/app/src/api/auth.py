...
async def get_current_role(
    ...
    # In Supabase JWT, role is usually 'authenticated', but metadata can have custom role.
...
def require_educator(user: User = Depends(get_current_user)) -> User:
    if user.role not in [UserRole.ADMIN, UserRole.SUPERADMIN, UserRole.TEACHER]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Teacher/Admin privilege required"
        )
    return user
...
def refresh_token(request: RefreshTokenRequest, db: Session = Depends(get_db)):
    ...
    access_token = create_access_token(data={"sub": str(user.id), "email": user.email, "role": user.role})
    new_refresh_token = create_refresh_token(data={"sub": str(user.id), "email": user.email, "role": user.role})
...
def get_profile_stats(
    ...
    if role_str == "parent":
        ...
        if student_id and student_id != current_user.id:
            raise HTTPException(
                status_code=403,
                detail="Access denied. Students can only view their own stats."
            )
    ...
    simulations_count = db.query(SimulationHistory).filter(
        SimulationHistory.user_id == target_user.id,
        SimulationHistory.is_active == True
    ).count()

    # 2. Formulas explored
    formulas_count = db.query(FormulaHistory).filter(
        FormulaHistory.user_id == target_user.id
    ).count()

    # 3. Chapter/topic progress (completed vs total)
    profile_obj = db.query(StudentProfile).filter(StudentProfile.user_id == target_user.id).first()
    mastered_count = 0
